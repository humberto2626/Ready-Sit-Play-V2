...
Each player asks the canine player to "Give me" for 1 point, "Drop it" 2 points and "Leave it" for 3 points.
  ...
  function handleCardClick(card: Card) {
    if (card.category === 'Action') {
      showActionInstruction(card);
    } else if (card.category === 'Challenge') {
      showChallengeInstruction(card);
    } else if (card.category === 'Mini Game') {
      showMiniGameInstruction(card);
    }
  }
</script>
  ...
  {#if activeCard}
    <div class="active-card-container">
      ...
      <div class="cards-row">
        ...
        {#if selectedChallengeCard}
          <div class="card open edge-challenge" style="position: relative; cursor: pointer;" on:click={() => handleCardClick(selectedChallengeCard)}>
